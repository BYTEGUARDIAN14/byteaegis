...
import { FileText, Search, BarChart3, Lock, Package, Cpu } from "lucide-react"
...
export default function Features() {
  ...
  const [baseColor, setBaseColor] = useState<[number, number, number]>([0.2314, 0.51, 0.957])
  const [glowColor, setGlowColor] = useState<[number, number, number]>([0.2314, 0.51, 0.957])

  const [dark, setDark] = useState<number>(theme === "dark" ? 1 : 0)

  useEffect(() => {
    setBaseColor([0.2314, 0.51, 0.957])
    setGlowColor([0.2314, 0.51, 0.957])
  ...
  return (
    <section id="services" className="text-foreground relative overflow-hidden py-8 sm:py-12 md:py-24 lg:py-32">
      ...
      <motion.div
        ...
        className="container mx-auto flex flex-col items-center gap-4 sm:gap-6 md:gap-12 px-4 sm:px-6"
      >
        <h2
          className={cn(
        ...
        <div>
            <div className="grid grid-cols-12 gap-4 justify-center">
              {/* Service 1: Secure Web & Desktop App Dev */}
              <motion.div
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="flex items-center gap-8">
                        ...
                        <div className="flex flex-col gap-3">
                          {["Design", "Build", "Deploy"].map((item, index) => (
                            <motion.div
                              key={`left-${index}`}
                            ...
                            >
                              <div className="w-4 h-4 flex items-center justify-center text-[#3b82f6]">
                                {index === 0 && <FileText className="w-3.5 h-3.5" />}
                                {index === 1 && <Search className="w-3.5 h-3.5" />}
                                {index === 2 && <BarChart3 className="w-3.5 h-3.5" />}
                              </div>
                              {item}
                            </motion.div>
                          ))}
                        ...
                        {/* Right Column */}
                        <div className="flex flex-col gap-3">
                          {["Secure", "Test", "Harden"].map((item, index) => (
                            <motion.div
                              key={`right-${index}`}
                              className="bg-white rounded px-3 py-2 flex items-center gap-2 text-black text-sm font-medium shadow-sm"
                              initial={{ opacity: 1, x: 0 }}
                            ...
                            >
                              <div className="w-4 h-4 flex items-center justify-center text-[#3b82f6]">
                                {index === 0 && <Lock className="w-3.5 h-3.5" />}
                                {index === 1 && <Package className="w-3.5 h-3.5" />}
                                {index === 2 && <Cpu className="w-3.5 h-3.5" />}
                              </div>
                              {item}
                            </motion.div>
                          ))}
                      ...
                      <svg width="350" height="350" viewBox="0 0 350 350" className="opacity-40">
                        <motion.path
                          ...
                          transition={{
                            ...
                            rotate: { duration: 20, repeat: isCliHovering ? Number.POSITIVE_INFINITY : 0, ease: "linear" },
                          }}
                        />
                      </svg>
                    </motion.div>
              ...
              {/* Service 2: Vulnerability Assessment & Code Auditing */}
              <motion.div
              ...
              >
                <div className="flex flex-col gap-4">
                  <h3 className="text-2xl leading-none font-semibold tracking-tight">Vulnerability Assessment &amp; Code Auditing</h3>
                  <div className="text-md text-muted-foreground flex flex-col gap-2 text-sm">
                    <p className="max-w-[460px]">
                      We review your codebase and identify security vulnerabilities, exposed credentials, insecure dependencies, and risky patterns. You get a full report with prioritized findings and clear remediation steps.
                    </p>
                  </div>
                </div>
                  ...
                  <h1 className="mt-8 text-center text-5xl leading-[100%] font-semibold sm:leading-normal lg:mt-12 lg:text-6xl">
                    <span className='bg-background relative mt-3 inline-block w-fit rounded-md border px-1.5 py-0.5 before:absolute before:top-0 before:left-0 before:z-10 before:h-full before:w-full before:bg-[url("/noise.gif")] before:opacity-[0.09] before:content-[""]'>
                      <ScrambleHover
                        ...
                        className="cursor-pointer bg-gradient-to-t from-[#3b82f6] to-[#60a5fa] bg-clip-text text-transparent"
              ...
              >
                <div className="flex flex-col gap-4">
                  <h3 className="text-2xl leading-none font-semibold tracking-tight">DevSecOps &amp; CI/CD Pipeline Setup</h3>
                  <div className="text-md text-muted-foreground flex flex-col gap-2 text-sm">
                    <p className="max-w-[460px]">
                      We set up automated security pipelines so every code push is scanned for vulnerabilities and secrets before reaching production. We work with GitHub Actions, GitLab CI, and Docker-based workflows.
                    </p>
                  </div>
                </div>
                    ...
                    <div className="relative rounded-2xl border border-white/10 bg-black/20 dark:bg-white/5 backdrop-blur-sm">
                      <div className="p-4">
                        <textarea
                          ...
                          placeholder="Describe your pipeline setup needs..."
                          value={inputValue}
                      ...
                      <div className="flex items-center justify-between px-4 pb-4">
                        <div className="flex items-center gap-3">
                          ...
                          <button className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#3b82f6] hover:bg-[#2563eb] transition-colors text-white font-medium">
              ...
              {/* Service 4: AI Integration for Security */}
              <motion.div
                className="group border-secondary/40 text-card-foreground relative col-span-12 flex flex-col overflow-hidden rounded-xl border-2 p-6 shadow-xl transition-all ease-in-out md:col-span-6 xl:col-span-6 xl:col-start-8"
